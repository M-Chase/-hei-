// pages/uploadCompus/uploadCompus.js
var app=getApp()
Page({
  ...
  submit: function () {
    ...
    if (!this.data.title) {
      ...
      if (img_arr.length != 0)
        that.uploadfile_img(formData, img_arr)
      else
  ...
  upimg: function () {
    var that = this;
    var img_arr = that.data.img_arr;
    const imgnum = Object.keys(img_arr).length;
    if (imgnum < 9) {
      wx.chooseImage({
  ...
  previewImg: function (e) {
    ...
    console.log(img_arr)
    wx.previewImage({
      current: img_arr[e.currentTarget.dataset.imgid],
      urls: img_arr
    })
  },
  /**
